...
RUN apt-get update && apt-get install --allow-change-held-packages --allow-downgrades --allow-remove-essential -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confold -fy -t bullseye-backports libc6 golang \
&& dpkg -i /nginx.deb \
&& rm -rf /nginx.deb \
